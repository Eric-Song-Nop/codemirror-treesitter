...
function App() {
  ...
  let parserLoadedRef = useRef(false);
  let frameRef = useRef(0);
  let [stats, setStats] = useState<EditorStats>(() => statsFromText(loadInitialDoc()));

  useEffect(() => {
    let parent = editorRef.current;
    if (!parent) return;

    let updateStats = (state: EditorState) => {
      window.cancelAnimationFrame(frameRef.current);
      frameRef.current = window.requestAnimationFrame(() => {
        setStats(readEditorStats(state, parserLoadedRef.current));
      });
    };
    ...
    if (!markdownDescription) {
      setStats((current) => ({ ...current, parser: "Unavailable", treeRoot: "missing" }));
    } else {
      void markdownDescription
        .load()
        .then((support) => {
          if (cancelled) return;
          parserLoadedRef.current = true;
          view.dispatch({
        ...
        .catch((error: unknown) => {
          console.error(error);
          setStats((current) => ({ ...current, parser: "Failed", treeRoot: "error" }));
        });
    }
  ...
  return (
    <main className="typora-shell">
      <header className="typora-chrome">
        <div>
          <p className="eyebrow">CodeMirror Tree-sitter</p>
          <h1>Typora Markdown</h1>
        </div>
        <dl className="status-strip" aria-label="Editor status">
          <div>
            <dt>Parser</dt>
            <dd>{stats.parser}</dd>
          </div>
          ...
          <div>
            <dt>Block</dt>
            <dd>{stats.activeBlock}</dd>
          </div>
        </dl>
      </header>
      <section className="writing-room" aria-label="Markdown editor workspace">
        <div className="paper">
        ...
        <aside className="inspector" aria-label="Document details">
          <dl>
            ...
            <div>
              <dt>Tasks</dt>
              <dd>
                {stats.checkedTasks}/{stats.tasks}
              </dd>
            </div>
          </dl>
        </aside>
...
function analyzeMarkdownTree(state: EditorState, activeLines: Set<number>): MarkdownAnalysis {
  let analysis: MarkdownAnalysis = {
    ...
    imageBlocks: new Map(),
    lines: new Map(),
    stats: { checkedTasks: 0, headings: 0, tasks: 0 },
    tableBlocks: [],
  };
...
function registerTaskMarker(state: EditorState, analysis: MarkdownAnalysis, node: SyntaxNode) {
  ...
  analysis.stats.tasks++;
  if (checked) analysis.stats.checkedTasks++;
...
function readEditorStats(state: EditorState, parserLoaded: boolean): EditorStats {
  if (parserLoaded) ensureSyntaxTree(state, state.doc.length, 25);
  let text = state.doc.toString();
  let words = text.trim() ? text.trim().split(/\s+/).length : 0;
  let markdownStats = parserLoaded
    ? analyzeMarkdownTree(state, getActiveLines(state)).stats
    : { checkedTasks: 0, headings: 0, tasks: 0 };
  let tree = syntaxTree(state);
  return {
    activeBlock: parserLoaded ? describeActiveBlock(state) : "Loading",
    characters: text.length,
    checkedTasks: markdownStats.checkedTasks,
    headings: markdownStats.headings,
    lines: state.doc.lines,
    parser: parserLoaded ? (syntaxTreeAvailable(state) ? "Ready" : "Parsing") : "Loading",
    tasks: markdownStats.tasks,
    treeRoot: tree.topNode.name || "document",
    words,
  };
}

function describeActiveBlock(state: EditorState) {
  let cursor = state.selection.main.head;
  let currentLine = state.doc.lineAt(cursor);
  let activeLines = new Set([currentLine.number]);
  let analysis = analyzeMarkdownTree(state, activeLines);
  let codeLineState = analysis.codeLines.get(currentLine.number);
  if (codeLineState) return codeLineState.boundary ? "Code fence" : "Code";

  let lineClasses = analysis.lines.get(currentLine.number)?.classes;
  if (lineClasses?.has("cm-md-task-line")) return "Task";
  if (lineClasses?.has("cm-md-table-line")) return "Table";
  if (lineClasses?.has("cm-md-heading-1")) return "Heading 1";
  if (lineClasses?.has("cm-md-heading-2")) return "Heading 2";
  if (lineClasses?.has("cm-md-heading-3")) return "Heading 3";
  if (lineClasses?.has("cm-md-heading-4")) return "Heading 4";
  if (lineClasses?.has("cm-md-heading-5")) return "Heading 5";
  if (lineClasses?.has("cm-md-heading-6")) return "Heading 6";
  if (lineClasses?.has("cm-md-list-line")) return "List";
  if (lineClasses?.has("cm-md-blockquote")) return "Quote";

  let node: SyntaxNode | null = syntaxTree(state).resolveInner(cursor, -1);
  while (node) {
    let label = labelMarkdownNode(node.name);
...
function labelMarkdownNode(name: string) {
  switch (name) {
    case "atx_heading":
    case "setext_heading":
      return "Heading";
...
function statsFromText(text: string): EditorStats {
  return {
    activeBlock: "Loading",
    characters: text.length,
    checkedTasks: 0,
