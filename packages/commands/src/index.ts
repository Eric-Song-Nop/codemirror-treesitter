...
import { EditorView, Direction, type Command, type KeyBinding } from "@codemirror/view";
import {
  syntaxTree,
  IndentContext,
  getIndentUnit,
...
export type { CommentTokens } from "./comment.js";
export {
  toggleComment,
  toggleLineComment,
  lineComment,
...
function moveByCharLogical(target: CommandTarget, forward: boolean) {
  return moveSel(target, (range) =>
    range.empty ? byCharLogical(target.state, range, forward) : rangeEnd(range, forward),
  );
}
...
function interestingNode(
  state: EditorState,
  node: SyntaxNode,
  bracketProp: NodeProp<readonly string[]>,
) {
...
function addCursorVertically(view: EditorView, forward: boolean) {
  let { state } = view,
    sel = state.selection,
    ranges: SelectionRange[] = state.selection.ranges.slice();
...
export const deleteGroupForwardWin: Command = (view) =>
  deleteBy(
    view,
    (range) => view.moveByChar(range, true, (start) => toGroupStart(view, range.head, start)).head,
  );
