...
const historyField_ = StateField.define({
  ...
  fromJSON(json) {
    return new HistoryState(
      json.done.map((event: unknown) => HistEvent.fromJSON(event)),
      json.undone.map((event: unknown) => HistEvent.fromJSON(event)),
    );
  },
});
...
class HistEvent {
  ...
  static fromJSON(json: any) {
    return new HistEvent(
      ...
      json.selectionsAfter.map((selection: unknown) => EditorSelection.fromJSON(selection)),
    );
  }
...
class HistoryState {
  constructor(
    ...
    private readonly prevUserEvent?: string,
  ) {}
