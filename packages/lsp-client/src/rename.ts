...
import { EditorView, showDialog, getDialog, type Command, type KeyBinding } from "@codemirror/view";
...
function doRename(view: EditorView, newName: string) {
  ...
  void plugin.client.withMapping((mapping) =>
    getRename(plugin, word.from, newName).then(
      (response) => {
        if (!response) return;
        for (let uri in response.changes) {
          let lspChanges = response.changes[uri],
            file = plugin.client.workspace.getFile(uri);
