...
/// source.
export function serverCompletion(
  config: {
...
function prefixRegexp(items: readonly lsp.CompletionItem[]) {
  let step = Math.ceil(items.length / 50),
    prefixes: string[] = [];
  for (let i = 0; i < items.length; i += step) {
    let item = items[i],
      text = item.textEdit?.newText || item.textEditText || item.insertText || item.label;
    if (!/^\w/.test(text)) {
  ...
  return new RegExp(
    "^(?:" +
      prefixes.map((RegExp as any).escape || ((s) => s.replace(/[^\w\s]/g, "\\$&"))).join("|") +
      ")?\\w*$",
  );
}
...
function shouldTriggerCompletion(
  plugin: LSPPlugin,
  character: string,
): "identifier" | "triggerCharacter" | null {
...
export const serverCompletionSource: CompletionSource = (context) => {
  ...
  return getCompletions(plugin, context.pos, completionContext, context).then(
    (result) => {
        ...
        options: result.items.map<Completion>((item) => {
          ...
          if (insertTextFormat == 2 /* Snippet */) {
            option.apply = (view, c, from, to) =>
              snippet(text.replace(/\$(\d+)/g, "${$1}"))(view, c, from, to);
...
function completionResultRange(
  ...
  let defaultRange = result.itemDefaults?.editRange,
  ...
  let range = defaultRange
    ? "insert" in defaultRange
      ...
      : defaultRange
    : item0.textEdit
      ? "range" in item0.textEdit
        ? item0.textEdit.range
        : item0.textEdit.insert
      : null;
