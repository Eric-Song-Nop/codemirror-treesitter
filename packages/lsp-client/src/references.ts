...
import { WorkspaceMapping } from "./client.js";
...
export const findReferences: Command = (view) => {
  ...
  let mapping = plugin.client.workspaceMapping(),
    passedMapping = false;
  getReferences(plugin, view.state.selection.main.head)
    .then(
      (response) => {
        if (!response) return;
        return Promise.all(
  ...
  return (view) => {
    ...
    for (let { file, range } of locs) {
      ...
      let view = file.getView(),
        line = (view ? view.state.doc : file.doc).lineAt(from);
      ...
      entry.appendChild(document.createElement("strong")).textContent = line.text.slice(
        from - line.from,
        to - line.from,
      );
    ...
    function showReference(index: number) {
      ...
      void Promise.resolve(
        file.uri == plugin.uri ? view : plugin.client.workspace.displayFile(file.uri),
      ).then((view) => {
        if (!view) return;
