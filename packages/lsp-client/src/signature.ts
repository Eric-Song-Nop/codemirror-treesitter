...
function getSignatureHelp(plugin: LSPPlugin, pos: number, context: lsp.SignatureHelpContext) {
  ...
  plugin.client.sync();
  return plugin.client.request<lsp.SignatureHelpParams, lsp.SignatureHelp | null>(
...
const signaturePlugin = ViewPlugin.fromClass(
  class {
    ...
    startRequest(plugin: LSPPlugin, context: lsp.SignatureHelpContext) {
      ...
      void getSignatureHelp(plugin, pos, context).then(
        (result) => {
          if (req.drop) return;
          if (result && result.signatures.length) {
            ...
            let active =
              same && context.triggerKind == 3 ? cur!.active : (result.activeSignature ?? 0);
        ...
        (err) => {
          if (context.triggerKind == 1 /* Invoked */)
            plugin.reportError("Signature request failed", err);
        },
      );
    }
