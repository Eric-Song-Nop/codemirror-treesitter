...
import { EditorView, ViewPlugin, showDialog, type ViewUpdate } from "@codemirror/view";
...
export class LSPPlugin {
  ...
  docToHTML(value: string | lsp.MarkupContent, defaultKind: lsp.MarkupKind = "plaintext") {
    let html = withContext(this.view, this.client.config.highlightLanguage, () =>
  ...
  update(update: ViewUpdate) {
    if (update.docChanged) this.unsyncedChanges = this.unsyncedChanges.compose(update.changes);
  }
