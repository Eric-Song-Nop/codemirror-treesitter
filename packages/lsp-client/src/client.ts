...
import { ChangeSet, ChangeDesc, MapMode, Text, type Extension } from "@codemirror/state";
import { Language } from "@codemirror-treesitter/language";
import { LSPPlugin, lspPlugin } from "./plugin.js";
import { toPosition, fromPosition } from "./pos.js";
import { Workspace, DefaultWorkspace, type WorkspaceFile } from "./workspace.js";
...
export class LSPClient {
  ...
  private readonly receiveMessageHandler = (message: string) => this.receiveMessage(message);
  ...
  connect(transport: Transport) {
    if (this.transport) this.transport.unsubscribe(this.receiveMessageHandler);
    this.transport = transport;
    transport.subscribe(this.receiveMessageHandler);
  ...
  plugin(fileURI: string, languageID?: string): Extension {
    return [lspPlugin.of({ client: this, uri: fileURI, languageID }), lspTheme, this.extensions];
  }
  ...
  private requestInner<Params, Result>(method: string, params: Params): Request<Result> {
  ...
  sync() {
    for (let { file, changes, prevDoc } of this.workspace.syncFiles()) {
      for (let mapping of this.activeMappings) mapping.addChanges(file.uri, changes);
      if (this.supportSync)
...
function contentChangesFor(
  ...
  if (!supportInc || file.doc.length < Sync.AlwaysIfSmaller) return [{ text: file.doc.toString() }];
...
function mergeCapabilities(base: any, add?: any) {
  ...
  for (let prop of baseProps)
    result[prop] =
      addProps.indexOf(prop) > -1 ? mergeCapabilities(base[prop], add[prop]) : base[prop];
  for (let prop of addProps) if (baseProps.indexOf(prop) < 0) result[prop] = add[prop];
  return result;
}
