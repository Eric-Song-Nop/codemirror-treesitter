...
function getDefinition(plugin: LSPPlugin, pos: number) {
  return plugin.client.request<lsp.DefinitionParams, lsp.Location | lsp.Location[] | null>(
...
function getDeclaration(plugin: LSPPlugin, pos: number) {
  return plugin.client.request<lsp.DeclarationParams, lsp.Location | lsp.Location[] | null>(
...
function getTypeDefinition(plugin: LSPPlugin, pos: number) {
  return plugin.client.request<lsp.TypeDefinitionParams, lsp.Location | lsp.Location[] | null>(
...
function getImplementation(plugin: LSPPlugin, pos: number) {
  return plugin.client.request<lsp.ImplementationParams, lsp.Location | lsp.Location[] | null>(
...
function jumpToOrigin(
  ...
  plugin.client.sync();
  void plugin.client.withMapping((mapping) =>
      ...
      (response) => {
        ...
        return (
          loc.uri == plugin.uri
            ...
            : plugin.client.workspace.displayFile(loc.uri)
        ).then((target) => {
          if (!target) return;
          let pos = mapping.getMapping(loc.uri)
