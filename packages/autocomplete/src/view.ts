  ...
  return (view: EditorView) => {
    ...
    if (by == "page" && (tooltip = getTooltip(view, cState.open.tooltip)))
      step = Math.max(
        2,
        Math.floor(
          tooltip.dom.offsetHeight / (tooltip.dom.querySelector("li") as HTMLElement).offsetHeight,
        ) - 1,
      );
    ...
    update(update: ViewUpdate) {
      ...
      let doesReset = update.transactions.some((tr) => {
        ...
        return (
          ...
          ((tr.selection || tr.docChanged) && !(type & UpdateType.SimpleInteraction))
        );
      });
      ...
      if (this.composing != CompositionState.None)
        for (let tr of update.transactions) {
          if (tr.isUserEvent("input.type")) this.composing = CompositionState.Changed;
