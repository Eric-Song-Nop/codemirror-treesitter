...
import { FuzzyMatcher, StrictMatcher } from "./filter.js";
import { completionTooltip, setSelectedEffect } from "./tooltip.js";
import { completionConfig, type CompletionConfig } from "./config.js";
...
function sortOptions(active: readonly ActiveSource[], state: EditorState) {
  ...
  let sections: null | CompletionSection[] = null,
    dynamicSectionScore: Record<string, number> | null = null;
          ...
          if ((match = matcher.match(option.label))) {
            ...
            if (typeof option.section == "object" && option.section.rank === "dynamic") {
              ...
              if (!dynamicSectionScore)
                dynamicSectionScore = Object.create(null) as Record<string, number>;
  ...
  update(tr: Transaction) {
    ...
    let active: readonly ActiveSource[] = sources.map((source) => {
      let value =
        ...
        new ActiveSource(
          source,
          this.active.some((a) => a.state != State.Inactive) ? State.Pending : State.Inactive,
        );
...
export class ActiveSource {
  ...
  update(tr: Transaction, conf: Required<CompletionConfig>): ActiveSource {
    ...
    let value: ActiveSource =
      type & UpdateType.Reset || (type & UpdateType.ResetIfTouching && this.touches(tr))
  ...
  map(_changes: ChangeDesc): ActiveSource {
    return this;
  }
