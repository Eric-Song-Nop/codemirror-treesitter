...
import { indentUnit } from "@codemirror-treesitter/language";
import { baseTheme } from "./theme.js";
import { pickedCompletion, type Completion } from "./completion.js";
...
class Snippet {
  ...
  instantiate(state: EditorState, pos: number) {
    ...
    let ranges = this.fieldPositions.map(
      (pos) =>
        new FieldRange(pos.field, lineStart[pos.line] + pos.from, lineStart[pos.line] + pos.to),
    );
    return { text, ranges };
  }
