...
import { StateEffect, type EditorState, type StateField } from "@codemirror/state";
...
class CompletionTooltip {
    ...
    this.dom.addEventListener("mousedown", (e: MouseEvent) => {
      ...
      if (e.target == this.list) {
        let move =
          ...
          e.clientY < (this.list.firstChild as HTMLElement).getBoundingClientRect().top
            ? this.range.from - 1
            : this.list.classList.contains("cm-completionListIncompleteBottom") &&
                e.clientY > (this.list.lastChild as HTMLElement).getBoundingClientRect().bottom
              ? this.range.to
              : null;
        if (move != null) {
  ...
  addInfoPane(content: NonNullable<CompletionInfo>, _completion: Completion) {
    ...
    if ((content as Node).nodeType != null) {
      ...
      let info = content as { dom: Node; destroy?(): void };
      wrap.appendChild(info.dom);
      this.infoDestroy = info.destroy ? () => info.destroy!() : null;
    }
