...
type Config = {
  ...
  mergeControls?:
    | boolean
    | ((type: "accept" | "reject", action: (event: MouseEvent) => void) => HTMLElement);
  overrideChunk?:
    | ((
        state: EditorState,
        chunk: Chunk,
...
export const computeChunks =
  Facet.define<(current: readonly Chunk[], tr: Transaction) => readonly Chunk[]>();

export const ChunkField = StateField.define<readonly Chunk[]>({
  create() {
    return null as unknown as readonly Chunk[];
  },
  update(current, tr) {
