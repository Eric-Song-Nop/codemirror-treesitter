...
import { highlightingFor, highlightTree, language } from "@codemirror-treesitter/language";
import { Chunk, defaultDiffConfig } from "./chunk.js";
import { computeChunks, ChunkField, mergeConfig } from "./merge.js";
import type { Change, DiffConfig } from "./diff.js";
import { decorateChunks, collapseUnchanged, changedText } from "./deco.js";
...
function deletionWidget(state: EditorState, chunk: Chunk, hideContent: boolean) {
  ...
  let buildDOM = (view: EditorView) => {
    ...
    if (lang && chunk.toA - chunk.fromA <= syntaxHighlightDeletionsMaxLength!) {
      let tree = lang.parser.parse(Text.of(text.split(/\r?\n/))),
        pos = 0;
  ...
  abort: if (linesA == linesB && linesA < 10) {
    ...
    for (let ch of chunk.changes) {
      if (ch.fromA < ch.toA) {
        ...
        deco.push(
          Decoration.widget({ widget: new InlineDeletion(deleted), side: -1 }).range(bB + ch.fromB),
        );
      }
      if (ch.fromB < ch.toB) {
