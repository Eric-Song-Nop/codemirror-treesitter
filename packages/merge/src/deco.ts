...
import { ChunkField, mergeConfig } from "./merge.js";
...
function getChunkDeco(view: EditorView) {
  ...
  let { side, highlightChanges, markGutter, overrideChunk } = view.state.facet(mergeConfig),
    isA = side == "a";
...
function compareSpacers(a: DecorationSet, b: DecorationSet) {
  ...
  while (iA.value) {
    if (
      iA.from != iB.from ||
      Math.abs((iA.value.spec.widget as Spacer).height - (iB.value!.spec.widget as Spacer).height) >
        1
    )
      return false;
    iA.next();
...
export function updateSpacers(a: EditorView, b: EditorView, chunks: readonly Chunk[]) {
  ...
  for (let chunkI = 0; ; chunkI++) {
    ...
    let endA = chunk ? chunk.fromA : a.state.doc.length,
      endB = chunk ? chunk.fromB : b.state.doc.length;
...
const CollapsedRanges = StateField.define<DecorationSet>({
  ...
  update(deco, tr) {
    ...
    for (let e of tr.effects)
      if (e.is(uncollapseUnchanged)) deco = deco.update({ filter: (from) => from != e.value });
    return deco;
  },
  provide: (f) => EditorView.decorations.from(f),
});
