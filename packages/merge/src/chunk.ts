...
import { presentableDiff, diffIsPrecise, type Change, type DiffConfig } from "./diff.js";
...
function toChunks(
  ...
  for (let i = 0; i < changes.length; i++) {
    ...
    chunks.push(
      new Chunk(chunk, fromA, Math.max(fromA, toA), fromB, Math.max(fromB, toB), precise),
    );
  }
  return chunks;
}
