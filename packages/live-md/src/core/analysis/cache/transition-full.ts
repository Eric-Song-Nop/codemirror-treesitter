...
export function transitionLeafAnalysisCache(input: {
  ...
  forEachLeafAnalysisCacheRecord(input.oldCache, (record) => oldRecords.push(record));
  let oldCandidates = mappedOldRecordCandidates(oldRecords, input.changes, input.yieldCheck, trace);
...
export function mappedOldRecordCandidates(
...
export function reusableOldRecord(
  oldDoc: Text,
  newDoc: Text,
...
export function normalizeRanges(ranges: readonly DocRange[], docLength: number) {
  let sorted = ranges
    .map((range) => ({
...
export function analysisInputWithInlineSession(
...
export function disposeInlineSession(session: MarkdownInlineAnalysisSession | null) {
  session?.dispose();
}
